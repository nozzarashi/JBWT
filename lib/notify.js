...
export async function notifyBatch(groups, errors = []) {
  if (!groups.length && !errors.length) return;

  const total = groups.reduce((n, g) => n + g.vacancies.length, 0);
  const header =
    total > 0
      ? `🆕 <b>Новые фронтенд-вакансии: ${total}</b>`
      : '✅ <b>Проверка завершена, новых вакансий нет</b>';
  ...
  if (errors.length) {
    const errLines = errors.map(
      (e) => `   • <b>${escapeHtml(e.displayName)}</b> — ${escapeHtml(e.message)}`
    );
    blocks.push(`⚠️ <b>Сломались парсеры (${errors.length}):</b>\n${errLines.join('\n')}`);
  }
